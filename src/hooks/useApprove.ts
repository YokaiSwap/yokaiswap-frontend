...
export const useSousApprove = (lpContract: Contract, sousId, earningTokenSymbol) => {
  ...
  const handleApprove = useCallback(async () => {
    try {
      ...
      dispatch(updateUserAllowance(sousId, godwokenAddress))
      if (tx) {
        toastSuccess(
          t('Contract Enabled'),
...
export const useVaultApprove = (setLastUpdated: () => void) => {
  ...
  const handleApprove = () => {
    cakeContract.methods
      ...
      .on('receipt', () => {
        toastSuccess(t('Contract Enabled'), t('You can now stake in the %symbol% vault!', { symbol: 'YOK' }))
        setLastUpdated()
        setRequestedApproval(false)
      })
      .on('error', (error) => {
...
export const useCheckVaultApprovalStatus = () => {
  ...
  useEffect(() => {
    const checkApprovalStatus = async () => {
      try {
        const response = await cakeContract.methods
          .allowance(godwokenAddress, cakeVaultContract.options.address)
    ...
    checkApprovalStatus()
  }, [godwokenAddress, cakeContract, cakeVaultContract, lastUpdated])
